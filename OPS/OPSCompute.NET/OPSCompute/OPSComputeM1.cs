        ...
        private Dictionary<string, string> ExtractM1DataToMessage(CDatM1 m1, string m1ID)
        {
            ...
            try
            {
                ...
                if (m1.GetOutResult() > 0)
                {
                    ...
                    elements.Add($"{m1ID}qall", (m1.GetCurrentAmountIsAllowed()?1:0).ToString());
                    elements.Add($"{m1ID}aft", m1.GetOutAddFreeMinutesQuantity().ToString());
                    elements.Add($"{m1ID}afm", m1.GetOutAddFreeMoneyQuantity().ToString());
                    elements.Add($"{m1ID}hfm", (m1.GetOutHistFreeMoneyUsed()?1:0).ToString());
