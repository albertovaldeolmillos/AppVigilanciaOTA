...
namespace OPS.Components.Data
{
	...
	public class CmpCreditCardsTransactionsDB : CmpGenericBase
	{
		...
		public int InsertConfirmedTrans (IDbTransaction tran,string strTransId, DateTime dtDate,int nOpeId,int nQuantity,int nUniId, string strOperInfo, string strTransRef, out int nTransId)
		{

			return Insert(tran,strTransId, dtDate, "", "", new DateTime(1900,1,1,0,0,0), 
				"", "",TRANS_UNCOMMITTED,nQuantity,nOpeId, nUniId, strOperInfo, out nTransId);
		}
		...
		public int UpdatePendingTrans (IDbTransaction tran, int nCCTId, DateTime dtDate, int nOpeId, int nQuantity, int nUniId, string strOperInfo)
		{
			int res=-1;

			res = Update( tran, nCCTId, dtDate, "", "", new DateTime(1900,1,1,0,0,0), 
				"", "", TRANS_UNCOMMITTED, nQuantity, nOpeId,  nUniId, strOperInfo);
		...
		public int ExistTransaction(string strTransId, string strTransRef, int nUniId)
		{
			Database d = DatabaseFactory.GetDatabase();
			return (Convert.ToInt32(d.ExecuteScalar(string.Format("select CCT_ID from CREDIT_CARDS_TRANSACTIONS where CCT_UNI_ID={0} and CCT_TRANS_ID = '{1}'", nUniId, strTransId))));
		}

		public int ExistTransactionRef(string strTransRef, int nUniId)
		{
			Database d = DatabaseFactory.GetDatabase();
			return (Convert.ToInt32(d.ExecuteScalar(string.Format("select CCT_ID from CREDIT_CARDS_TRANSACTIONS where CCT_UNI_ID={0} and CCT_REFERENCE = '{1}'", nUniId, strTransRef))));
		}
		...
		public int GetTransactionState(int nTransId)
		{
			Database d = DatabaseFactory.GetDatabase();
			return (Convert.ToInt32(d.ExecuteScalar(string.Format("select CCT_STATE from CREDIT_CARDS_TRANSACTIONS where CCT_ID={0}", nTransId))));
		}
		...
		private int Insert(IDbTransaction tran, string strTransId, DateTime dtDate, string sCCNumber, string sCCName, DateTime dtExprtnDate,
			string strCCCodServ, string strCCDiscData, int nState, int nQuantity, int nOpeId, int nUniId, string strOperInfo, string strTransRef,
			out int nTransId)
		{
			Database d = null;                  // Database
			ILogger localLogger = null;         // Logger to trace
			...
			try
			{
				...
				if (iNumRegs == 0)
				{
					...
					string ssql = " INSERT INTO CREDIT_CARDS_TRANSACTIONS (" +
						...
						"@CREDIT_CARDS_TRANSACTIONS.CCT_UNI_ID@," +
						"@CREDIT_CARDS_TRANSACTIONS.CCT_OPER_INFO@," +
						"@CREDIT_CARDS_TRANSACTIONS.CCT_REFERENCE@) ";
		...
		private int Update (IDbTransaction tran, int nCCTId, DateTime dtDate, string sCCNumber, string sCCName, DateTime dtExprtnDate, 
			string strCCCodServ, string strCCDiscData,int nState,int nQuantity,int nOpeId, int nUniId, string strOperInfo )
		{
			Database d = null;					// Database
			ILogger localLogger = null;			// Logger to trace
			IDbConnection con = null;			// Connection
			...
			try
			{
				...
				string ssql = " UPDATE CREDIT_CARDS_TRANSACTIONS SET "+
					"CCT_OPE_ID = @CREDIT_CARDS_TRANSACTIONS.CCT_OPE_ID@,"+  
					"CCT_QUANTITY = @CREDIT_CARDS_TRANSACTIONS.CCT_QUANTITY@,"+  
					"CCT_NUMBER = @CREDIT_CARDS_TRANSACTIONS.CCT_NUMBER@,"+  
					"CCT_NAME = @CREDIT_CARDS_TRANSACTIONS.CCT_NAME@,"+    
					"CCT_EXPRTN_DATE = @CREDIT_CARDS_TRANSACTIONS.CCT_EXPRTN_DATE@,"+
					"CCT_STATE = @CREDIT_CARDS_TRANSACTIONS.CCT_STATE@,"+   
					"CCT_STATE_DATE = @CREDIT_CARDS_TRANSACTIONS.CCT_STATE_DATE@,"+
					"CCT_CODSERV = @CREDIT_CARDS_TRANSACTIONS.CCT_CODSERV@,"+ 
					"CCT_DISC_DATA = @CREDIT_CARDS_TRANSACTIONS.CCT_DISC_DATA@,"+
					"CCT_UNI_ID = @CREDIT_CARDS_TRANSACTIONS.CCT_UNI_ID@,"+
					"CCT_OPER_INFO = @CREDIT_CARDS_TRANSACTIONS.CCT_OPER_INFO@ "+
					" WHERE CCT_ID = @CREDIT_CARDS_TRANSACTIONS.CCT_ID@";
				
				if(localLogger != null)
				{
					localLogger.AddLog("[CmpCreditCardsTransactionsDB  ]: Executing...",LoggerSeverities.Debug);
					localLogger.AddLog("[CmpCreditCardsTransactionsDB  ]" + ssql,LoggerSeverities.Debug);
					localLogger.AddLog("[CmpCreditCardsTransactionsDB  ] TransId: " + nCCTId.ToString(),LoggerSeverities.Debug);
					try
					{
						if (sCCNumber.Length>=8)
						{
