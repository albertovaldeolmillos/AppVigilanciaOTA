...
namespace OPS.Comm.Becs.Messages
{
	...
	public sealed class Msg07 : MsgReceived, IRecvMessage
	{
		...
		public const int DEF_BIN_FORMAT_CORTE_INGLES = 9;
		public const int DEF_BIN_FORMAT_INGENICO = 10;
		public const int DEF_BIN_FORMAT_SPIRE = 11;
		public const int DEF_BIN_FORMAT_PAYTEF = 12;

		public const int DEF_BIN_FORMAT_KUTXA_ONLINE = 21;
		...
		public System.Collections.Specialized.StringCollection Process()
		{
			...
			try
			{
				...
				if (logger != null)
					logger.AddLog("[Msg07:Process - v1.1]", LoggerSeverities.Debug);
				...
				else if (_cardType==DEF_CARD_TYPE_CREDIT_CARD)
				{
					...
					else
					{
						...
						string strUseOnlineEbin = cmpParam.GetParameter("P_USE_ONLINE_EBIN");
						bool bCheckBines=true;
						bool bOnlineBinCheck = false;
						...
						if (strUseOnlineEbin == "")
						{
							bOnlineBinCheck = false;
						}
						else
						{
							bOnlineBinCheck = (strUseOnlineEbin != "0");
						}
						...
						bCheckBines = (bCheckBines&&
							...
							(iBinFormat != DEF_BIN_FORMAT_CORTE_INGLES) &&
							(iBinFormat != DEF_BIN_FORMAT_INGENICO) &&
							(iBinFormat != DEF_BIN_FORMAT_SPIRE) &&
							(iBinFormat != DEF_BIN_FORMAT_PAYTEF));
							...
							if (bCheckBines)
							{
								...
								switch(iBinFormat)
								{
									case DEF_BIN_FORMAT_LA_CAIXA:
										...
										break;			
									case DEF_BIN_FORMAT_KUTXA_ONLINE:

										if (!BinesKutxaOnline(logger))
										{
											responseResult = DEF_CREDIT_CARD_BIN_NOT_FOUND;
										}
										else
		...
		private bool BinesKutxaOnline(ILogger logger)
		{
			bool bRet = false;

			int nCurLen = 0;
			bool bFoundEbin = false;
			string strBin = "";
			...
			try
			{
				...
				try
				{
					// First find starting point for search by length
					DBCon.Open();
					cmd.Connection = (OracleConnection)DBCon;
					cmd.CommandText = String.Format("select nvl(max( ec_bin_length ), 0) from ebin_cards_online");
					nCurLen = Convert.ToInt32(cmd.ExecuteScalar());

					while (nCurLen > 0 && !bFoundEbin)
					{
						// Search the ebins by cycling through them by length, starting with the longest ones
						cmd.CommandText = String.Format("select ec_bin from ebin_cards_online " +
							"where ec_bin_length = '{0}' ", nCurLen);

						dr = cmd.ExecuteReader();

						while (dr.Read() && !bFoundEbin)
						{
							strBin = dr.GetString(dr.GetOrdinal("ec_bin"));
							bFoundEbin = (_szCCNumber.Substring(0, nCurLen) == strBin.Substring(0, nCurLen));
						}
					...
					{
						if (bFoundEbin)
							logger.AddLog("[Msg07:Process]: Found ebin " + strBin + " for Credit Card " + _szCCNumber, LoggerSeverities.Debug);
						else
							logger.AddLog("[Msg07:Process]: Credit Card " + _szCCNumber + " isn't in bin list", LoggerSeverities.Debug);
					}
				}
