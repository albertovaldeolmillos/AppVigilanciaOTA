	...
	public sealed class Msg02 : MsgReceived, IRecvMessage
	{
		...
		*/
		...
		public System.Collections.Specialized.StringCollection Process()
		{
			...
			logger = DatabaseFactory.Logger;
			if(logger != null)
				logger.AddLog("[Msg02:Process - v1.1]",LoggerSeverities.Debug);
						...
						if (nInsOperRdo == 0)
						{
							...
							try
							{
								if (_szCCNumber != "")
								{
									...
									else if (iBinFormat == Msg07.DEF_BIN_FORMAT_EMV_TAS || iBinFormat == Msg07.DEF_BIN_FORMAT_INGENICO
										|| iBinFormat == Msg07.DEF_BIN_FORMAT_SPIRE || iBinFormat == Msg07.DEF_BIN_FORMAT_PAYTEF)
									{
										int iTransId = -1;
									...
									else if (iBinFormat == Msg07.DEF_BIN_FORMAT_CORTE_INGLES)
									{
										// First try to update existing transaction received via M6
										// First search is by OTS Transaction ID
										// OTS Transaction ID is received in CCDResponseId field and ECI Transaction Reference is received in CCDTransId field
										int iCCTId = cmpCreditCardsTransactionsDB.ExistTransaction(_szCCDResponseId, _szCCDTransId, _unitId);

										// Existing transaction is found
										if (iCCTId > 0)
										{
											if (logger != null)
												logger.AddLog("[Msg02:Process]: CC Transaction found: " + iCCTId.ToString(), LoggerSeverities.Debug);

											int nState = cmpCreditCardsTransactionsDB.GetTransactionState(iCCTId);

											if (nState == 10)
											{
												if (cmpCreditCardsTransactionsDB.UpdatePendingTrans(tran, iCCTId, _date, nNewOperationID, Convert.ToInt32(_quantity), _unitId, _vehicleId) < 0)
									{
										RollbackTrans(tran);
										if (logger != null)
														logger.AddLog("[Msg02:Process]:ERROR ON UPDATE", LoggerSeverities.Debug);
										return ReturnNack(NackMessage.NackTypes.NACK_ERROR_BECS);
												}
												else
												{
													if (logger != null)
														logger.AddLog("[Msg02:Process]: UPDATE RESULT OK", LoggerSeverities.Debug);
												}
											}
											else
											{
												if (logger != null)
													logger.AddLog("[Msg02:Process]: CC Transaction already confirmed", LoggerSeverities.Debug);
											}
										}
										else
										{
											// Next search is by CCDResponseId
											if (logger != null)
												logger.AddLog("[Msg02:Process]: CC Transaction not found using OTS Transaction ID: " + _szCCDResponseId, LoggerSeverities.Debug);

											// OTS Transaction ID is received in CCDResponseId field and ECI Transaction Reference is received in CCDTransId field
											// Must subtract 2 from received ECI Transaction Reference
											if (_szCCDTransId.Length > 0)
											{
												int iTransRef = Convert.ToInt32(_szCCDTransId, 10);
												iTransRef -= 2;
												string szNewTransRef = iTransRef.ToString().PadLeft(6, '0');
												if (logger != null)
													logger.AddLog("[Msg02:Process]: Searching for CC Transaction using ECI Transaction Reference: " + szNewTransRef, LoggerSeverities.Debug);

												iCCTId = cmpCreditCardsTransactionsDB.ExistTransactionRef(szNewTransRef, _unitId);
											}
											...
											{
												if (logger != null)
													logger.AddLog("[Msg02:Process]: CC Transaction not found using ECI Transaction Reference: " + _szCCDTransId, LoggerSeverities.Debug);

												// If no existing transaction is found then insert new transaction
												int iTransId = -1;
												if (cmpCreditCardsTransactionsDB.InsertConfirmedTrans(tran, _szCCDResponseId, _date, nNewOperationID, Convert.ToInt32(_quantity), _unitId, _vehicleId, _szCCDTransId, out iTransId) < 0)
												{
													RollbackTrans(tran);
													if (logger != null)
														logger.AddLog("[Msg02:Process]:ERROR ON INSERT", LoggerSeverities.Debug);
