...
namespace OPS.Comm.Becs.Messages
{
	...
	/// <u>Unit (PDA) ID</p>
	/// </summary>
	public class Msg50 : MsgReceived, IRecvMessage
	{
		...
		private static int M50_RESULT_NO_TARIFF = 0;
		...
		public StringCollection Process()
		{
			...
			else if(_pedestrian == 0)
			{
				...
				if (bExistUnit)
				{
					...
					if (((bIsResident) || (bIsVIP)) && (!TicketObligatorio(-1)))
					{
						_result = M50_RESULT_OK;
					...
					if (_result==M50_RESULT_MUST_FINE || _result == M50_RESULT_NO_TARIFF)
					{
						if (!TicketObligatorio(-1))
						{
							_result=M50_RESULT_OK;
						...
						else
						{
							...
							if (bIsResident)
							{
								iArtId = GetResArtId(_vehicleId, _groupId);
								if (!TicketObligatorio(iArtId))
								{
									_result = M50_RESULT_OK;
									dtInitial = dtMaxVIPResident;
								}
							...
							else
							{
								if (bIsVIP)
								{
									iArtId = GetVipArtId(_vehicleId, _groupId);
									if (!TicketObligatorio(iArtId))
									{
										_result = M50_RESULT_OK;
										dtInitial = dtMaxVIPResident;
		...
		private bool TicketObligatorio( int iArtId )
		{
			bool bRet=true;
			int iCurArtId = -1;
			...
			try
			{
				...
				try
				{
					String strSQL = String.Format("select mati_is_mandatory from mandatory_ticket where mati_dart_id={0} and mati_grp_id={1}", iCurArtId, _groupId );
		...
		private int GetResArtId(string strPlate, int iGroupId)
		{
			int iArtId = -1;
			...
			try
			{
				...
				try
				{
					String strSQL = String.Format("select res_dart_id from residents where res_vehicleid='{0}' and res_grp_id={1}", strPlate, iGroupId);
					OracleCommand cmd = new OracleCommand(strSQL, (OracleConnection)DBCon);
					iArtId = Convert.ToInt32(cmd.ExecuteScalar());
		...
		private int GetVipArtId(string strPlate, int iGroupId)
		{
			int iArtId = -1;
			...
			try
			{
				...
				try
				{
					String strSQL = String.Format("select vip_dart_id from vips where vip_vehicleid='{0}' and vip_grp_id={1}", strPlate, iGroupId);
					OracleCommand cmd = new OracleCommand(strSQL, (OracleConnection)DBCon);
					iArtId = Convert.ToInt32(cmd.ExecuteScalar());
